...
red_low_range = np.array([160,25,20])
red_high_range = np.array([179+25,255,255])
...
while True:
    ...
    if warp is not None:
        if key == 103:
            detect, detected_coordinates = detectObjects(warp, green_low_range, green_high_range)
        if key == 114:
            detect, detected_coordinates = detectObjects(warp, red_low_range, red_high_range)
        if key == 98:
            detect, detected_coordinates = detectObjects(warp, blue_low_range, blue_high_range)
        
    
    # colTestMask = frame.copy()
    # colTestMask = colourQuant(colTestMask)
    # colTestMask = cv.cvtColor(colTestMask,cv.COLOR_BGR2HSV)
    # colTestMask = cv.inRange(colTestMask,blue_low_range, blue_high_range)
